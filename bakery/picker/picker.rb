...
conn = conn = Bunny.new host: ENV['RABBITMQ_HOST'] || 'rabbitmq', user: ENV['RABBITMQ_USERNAME'], pass: ENV['RABBITMQ_PASSWORD']
conn.start
...
begin
    sweet_done_queue.subscribe(:manual_ack => true, :block => true) do |delivery_info, properties, payload|
      ...
      RestClient.post "http://counter/v1/orders/#{task['id']}/sweet/done", {}, :content_type => :json, :accept => :json
